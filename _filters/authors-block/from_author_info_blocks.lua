...
local function create_affiliations_blocks(affiliations)
  local affil_lines = List:new(affiliations):map(
    function (affil, i)
      ...
      local function add_part(field)
        if not field then
          return
        end
        local value = stringify(field)
        if value and value ~= '' then
          table.insert(parts, value)
        end
      end

      add_part(affil.department)
      add_part(affil.name)
      add_part(affil.city)
      add_part(affil.country)

      local text = table.concat(parts, ', ')
      local info_inlines
      if text ~= '' then
        info_inlines = List:new{pandoc.Str(text)}
      else
        info_inlines = List:new{}
      end

      return num_inlines .. info_inlines
    end
  )
